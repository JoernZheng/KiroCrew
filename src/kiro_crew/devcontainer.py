...
def assert_no_sensitive_host_mounts(cfg: dict, project_dir: str | Path) -> None:
    ...
    ``source=...`` form and object form), ``workspaceMount``, and the raw docker
    flags in ``runArgs``. ``${localEnv:VAR}`` and ``${localWorkspaceFolder}``
    are expanded first, since the escape would otherwise just be spelled with a
    variable.
    ...
    for raw_source in _collect_host_mount_sources(cfg):
        source = _expand_devcontainer_vars(raw_source, project_dir)
        if not source or not os.path.isabs(source):
            # Named volumes and relative targets are not host paths.
            continue
...
def _expand_devcontainer_vars(value: str, project_dir: str | Path) -> str:
    """Expand the spec variables that can name a host path."""
    out = value.replace("${localWorkspaceFolder}", str(project_dir))

    def _local_env(m: re.Match[str]) -> str:
        return os.environ.get(m.group(1), "")

    return re.sub(r"\$\{localEnv:([A-Za-z_][A-Za-z0-9_]*)\}", _local_env, out)
    ...
    if isinstance(args, list):
        ...
        for i, arg in enumerate(flags):
            ...
            if arg in ("-v", "--volume") and i + 1 < len(flags):
                found.append(flags[i + 1].split(":", 1)[0])
            elif arg.startswith(("--volume=", "-v=")):
                found.append(arg.split("=", 1)[1].split(":", 1)[0])
...
def config_digest(config_path: Path) -> str:
    ...
    assert_no_sensitive_host_mounts(_cfg_obj, _project_root_of(config_path))
...
def config_preview(project_dir: str | Path) -> dict:
    ...
    assert_no_sensitive_host_mounts(parsed, _project_root_of(cfg))
