...
5. **RESOLVED (recommendation) — neither of the two, and not bare `rev-parse`
   either.** The question assumed the choice was between
   `os.path.exists(".git")` (accepts a linked worktree, today's HTTP behavior) and
   `Path(".git").is_dir()` (rejects it, today's CLI behavior). Both are
   filesystem-shape guesses at a question git can answer directly, and #1734 left
   the divergence in place by keeping `exists` on the check path.

   The obvious git-native replacement — `git rev-parse --git-dir` — is **wrong
   here, and worse than what it replaces on one axis**: it succeeds for any
   directory whose *ancestor* is a repository. The two rejected options are
   anchored to the install root; `--git-dir` is not. A wheel or source install
   nested under an unrelated checkout (a venv inside a project tree, a home
   directory that is itself a dotfiles repo) would classify as a git checkout —
   and while the boot-time git auto-apply remains armed, Phase 1b built on that
   would `git reset` a tree that has nothing to do with the install.

   So Phase 1b should use the git-native answer **with the anchor restored**:
   ...
   The behavior change relative to today stands — `kirocrew update` starts
   accepting linked worktrees it currently refuses — but it is now a deliberate
   widening to git's own semantic rather than an inherited accident. Cost: one
   subprocess where there was a `stat`, on a path that already shells out to git
