...
// Clear a previous run's output BEFORE recording. Playwright names its file by
// a random id, so the post-run scan cannot tell a stale `raw.webm` from a fresh
// capture — it would rename the old one over itself and report last week's video
// as evidence for today's diff.
for (const stale of readdirSync(OUT).filter(f => f.endsWith('.webm'))) {
